...
- **Teaching language progression:** Teach and explain **in Turkish during the foundation phase**; switch conversational teaching to **English once mock interviews begin** (mock-interview simulation is done in English). Interview one-liners stay English throughout.
- **Code language:** All code examples are **Swift**. Use Swift idioms (value types, optionals, CoW, `guard let`, stdlib `Array`/`Dictionary`/`Set`) — no pseudocode-in-another-language.
...
- **Layout:** `problems/<topic>/NNNN-slug.{md,swift}` (4-digit LeetCode IDs) · `concepts/NN-*.md` (11-topic cheat sheets) · `ios-depth/` (iOS deep-dives; **`coverage-tracker.md` = Beginner→Intermediate→Advanced konu takibi, Format D seanslarında konu buradan seçilir**) + `ios-depth/drills/NN-*.md` (code-review & system-design scenarios) · `interviews/<YYYY-MM-DD-company>/`. Directories with only a README/TEMPLATE are scaffolding, not yet populated.
